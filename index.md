...
1.  A 类地址：默认子网掩码是 255.0.0.0
    8bit 网络 ID，第一位必须以 0（二进制） 开头，24bit 主机 ID
...
2. 动态转换
   定义外部地址池，动态随机转换。私网 IP 和公网 IP 不确定时，可指定内部地址集和外部地址集，这样集合内的私网 IP 可随机转换为集合内的公网 IP。
   当 ISP 提供的合法 IP 地址略少于网络内部的计算机数量时。可以采用动态转换的方式。
   一对一转换
3. PAT （ Port Address Translation）
   多对一转换，最大程度节约公网 资源
   采用端口多路复用方式，通过端口号标识不同的数据流（改变外出数据包的源端口并进行端口转换，根据源端口标识是哪台主机，然后当数据返回时，根据记录的映射关系，返回给相应主机）
   目前应用最广泛的 实现方式
...
结构： 帧首部（含帧开始符-SOH） == 帧的数据部分 == 帧尾部（含帧结束符-EOT）

- 帧（Frame）的数据部分：
  大多数情况下就是网络层传递下来的数据包（IP 数据包，Packet）
...
网络层数据包,（IP 数据包，Packet）由首部、数据 2 部分组成。这里介绍的是 IPv4 的情况，IPv6 在后文描述。
...
占 16 位，首部的长度+数据的长度，类比于 IP 协议首部的 Total Length
...
- 客户端的源端口是临时开启的随机端口。
  它的作用是什么呢，比方说客户端不同的应用程序发了两个 UDP 请求，当数据回传时，无法区分返回的数据是哪次请求的。而处理机制就是根据随机端口来的，以随机端口为标志标记本条 UDP 请求。请求结束后端口就会关闭。
...
- 几个缩写：
  ...
  9. 连续收到 4 个 TCP 数据段，返回确认报文
     ...
     - 服务端
       ...
       客户端发送建立连接请求，但是此请求因网络延迟没有到达服务端。一定时间后，客户端重发，这次很顺利双方顺利完成建立连接、数据交换、释放连接。此时之前迟到的请求连接到达服务端。
...
- 客户端首先会访问最近的一台 DNS 服务器（也就是客户端自己配置的 DNS 服务器，一般也可以叫本地 DNS 服务器）
...
- 上级 DNS 服务器记录了下一级 DNS 服务器的 IP 地址
  比如本地 DNS 服务器缓存中没有，就将包发给根域名服务器。根域名服务器下记录了顶级域名的 IP 地址。比如.com .gov 等然后.com 下又记录了二级域名的 IP 地址，比如 baidu.com geogle.com 等就这样一级一级最终找到目标 DNS。 这样分层主要是因为万维网中的域名太多了，不可能又一台服务器可以放下所有域名。分层实际上就是只保存树结构数据的一层或几层的数据。
- 全球一共 13 台 IPv4 的 DNS 根域名服务器、25 台 IPv6 的 DNS 根域名服务器

它的具体过程如下图所示：
![DNS查找](./iamges/DNS查找.png)

查找过程都是通过本地 DNS 服务器完成的。本地 DNS 服务器发现缓存中没有，就会和根域名服务器通信，根域名服务器根据你请求的域名的顶级域名，返回该顶级域名服务器 IP 地址。然后本地 DNS 服务器再去请求顶级域名服务器，顶级域名服务器再根据域名返回二级域名服务器地址，以此类推，直到找到最终的域名位为止。
...
- 响应报文：
  ...
  12. Origin：发起一个针对跨域资源共享的请求，即跨域请求时浏览器会带上，根据服务器是否允许该地址判断，若不允许则浏览器abort请求并报错
...
- HTTPS 是在 HTTP 的基础上使用 SSL/TLS 来加密报文（整个个 HTTP 报文，不仅限于消息体），对窃听和中间人攻击提供合理的防护。
...
RARP：逆地址解析协议。使用与 ARP 相同的报文结构，作用于 ARP（通过 IP 地址获取 MAC 地址） 相反，用于将 MAC 地址转换为 IP 地址（根据 MAC 获取 IP）。后来被 BOOTP（HDCP 为其升级版）、DHCP 所取代。
...
相比 IPv4 更加简洁一些，两者的对比如下图所示：
![IPv6首部格式](./iamges/IPV4和IPv6首部对比.png)

IPv6 有 40 字节的固定首部，具体格式如下图：
...
- Traffic Class：交通类别
  ...
  作用类似于 IPv4 的 Differentiated Services Field（区分服务） 和 QOS。
